...
import { n as stegaClean } from "./stegaClean-D3ZaFDll.js";
...
function _fetch(client, httpRequest, _stega, query, _params = {}, options = {}) {
	...
	return stega.enabled ? $request.pipe(combineLatestWith(from(import("./stegaEncodeSourceMap-BY7ByzEt.js").then(function(n) {
...
function _getDocument(client, httpRequest, id, opts = {}) {
	return _requestObservable(client, httpRequest, {
		...
		tag: opts.tag,
		signal: opts.signal
	}).pipe(filter(isResponse), map((event) => event.body.documents && event.body.documents[0]));
}
...
var hasDataConfig = (client) => client.config().dataset !== void 0 && client.config().projectId !== void 0 || client.config()["~experimental_resource"] !== void 0, isQuery = (client, uri) => hasDataConfig(client) && uri.startsWith(_getDataUrl(client, "query")), isMutate = (client, uri) => hasDataConfig(client) && uri.startsWith(_getDataUrl(client, "mutate")), isDoc = (client, uri) => hasDataConfig(client) && uri.startsWith(_getDataUrl(client, "doc", "")), isListener = (client, uri) => hasDataConfig(client) && uri.startsWith(_getDataUrl(client, "listen")), isHistory = (client, uri) => hasDataConfig(client) && uri.startsWith(_getDataUrl(client, "history", "")), isData = (client, uri) => uri.startsWith("/data/") || isQuery(client, uri) || isMutate(client, uri) || isDoc(client, uri) || isListener(client, uri) || isHistory(client, uri);
...
var pick = (obj, props) => props.reduce((selection, prop) => (typeof obj[prop] > "u" || (selection[prop] = obj[prop]), selection), {}), eventSourcePolyfill = defer(() => import("./browser-yT2vdjvc.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1))).pipe(map(({ default: EventSource2 }) => EventSource2), shareReplay(1));
...
function _listen(query, params, opts = {}) {
	...
	return withCredentials && (esOptions.withCredentials = !0), token && (esOptions.headers = { Authorization: `Bearer ${token}` }), connectEventSource(() => (typeof EventSource > "u" || esOptions.headers ? eventSourcePolyfill : of(EventSource)).pipe(map((EventSource2) => new EventSource2(uri, esOptions))), listenFor).pipe(reconnectOnConnectionFailure(), filter((event) => listenFor.includes(event.type)), map((event) => ({
		type: event.type,
...
var LiveClient = class {
	...
	*/
	events({ includeDrafts = !1, tag: _tag } = {}) {
		...
		const esOptions = {};
		includeDrafts && token && (esOptions.headers = { Authorization: `Bearer ${token}` }), includeDrafts && withCredentials && (esOptions.withCredentials = !0);
		...
		const events = connectEventSource(() => (typeof EventSource > "u" || esOptions.headers ? eventSourcePolyfill : of(EventSource)).pipe(map((EventSource2) => new EventSource2(url.href, esOptions))), [
			...
			return event;
		})), observable2 = concat(fetchObservable(url, {
			method: "OPTIONS",
			mode: "cors",
		...
		})), events).pipe(finalize(() => eventsCache.delete(key)), shareReplayLatest({ predicate: (event) => event.type === "welcome" }));
...
var ProjectsClient = class {
	...
	*/
	getById(projectId2) {
		return resourceGuard("projects", this.#client.config()), lastValueFrom(_request(this.#client, this.#httpRequest, { uri: `/projects/${projectId2}` }));
	}
};
...
//#endregion
export { BasePatch, BaseTransaction, ChannelError, ClientError, ConnectionFailedError, CorsOriginError, DisconnectError, MessageError, MessageParseError, ObservablePatch, ObservableSanityClient, ObservableTransaction, Patch, SanityClient, ServerError, Transaction, connectEventSource, createClient, deprecatedCreateClient as default, requester, c as unstable__adapter, f as unstable__environment, validateApiPerspective };
